...
def OnRefresh(flags):
    ...
    if(HW_ChannelEvent & flags):
        prn(lvlA, 'channel event', _ChannelCount, channels.channelCount())
        UpdateChannelMap()  
...
def HandleProgressBar(padNum):
    ...
    transport.setSongPos(newSongPos)

    if(_AltHeld):
        offs = pdProgress.index(padNum) + 1
        arrangement.addAutoTimeMarker(arrangement.currentTime(1), "#" + str(offs))
...
def HandlePadMode(event):
    ...
    elif(not _AltHeld) and (_ShiftHeld): # Shift modes
        _isShiftMode = True 
        _isAltMode = False
        prn(lvl0, 'Shift Modes not ready')
...
def UpdateMarkerMap():
    ...
    while(markerNum > prevNum):
        ...
        markerTime = arrangement.currentTime()
...
def NavSnap(val):
    ...
    setSnapMode( newMode )  
    DisplayTextAll('Snap:', SnapModesText[newMode], '')   
